                ...
                else
                {
                    switch (action)
                    {
                        ...
                        case Action.SendBitmap:
                            pictureBox1.SizeMode = PictureBoxSizeMode.Zoom; // Set the SizeMode
        ...
        private async void Form1_Load(object sender, EventArgs e)
        {
          ...
          .Build();
            ...
            */
            ...
            _connection.On<List<string>>("ConnectionList", async (connectionIds) =>
            {
                ...
                try
                {
                    ...
                    {
                        HttpResponseMessage response = await client.GetAsync("http://localhost:5286/api/Chat/GetUsers");
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            List<string> userIds = users.Select(user => user.userId).ToList();

                            // Update the connection list with the retrieved user IDs
                            UpdateConnectionList(userIds);
                        }
                        else
                        {
                            // Handle unsuccessful HTTP response
                            Console.WriteLine("Failed to retrieve users from the API. Status code: " + response.StatusCode);
                        }
                    }
                }
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine("An error occurred while fetching users from the API: " + ex.Message);
                }
            });
            */
...
public byte[] BitmapToByteArray(Bitmap bitmap)
    {
        ...
        public async Task Sender(string ApiServer)
        {
            try
            {
                ...
                string connId = await GetConnectionId(selectedConnectionId);
                ...
                //  await _connection.SendAsync("SendApiLink", OneConnAtATime, ApiServer);
                await _connection.SendAsync("SendApiLink", connId, ApiServer);
            }
            catch (Exception ex)
            {
        ...
        private async Task<string> GetConnectionId(string userId)
        {
            ...
            {
                string url = $"http://localhost:5286/api/Chat/GetOneUser?userId={userId}";
                HttpResponseMessage response = await client.GetAsync(url);
                if (response.IsSuccessStatusCode)
                {
                    string connId = await response.Content.ReadAsStringAsync();
                    return connId;
                }
                else
                {
        ...
        private async void btnSignal_Click(object sender, EventArgs e)
        {
